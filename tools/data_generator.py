...
class DataGenerator:
    def __init__(self, seed: int = 42, base_timestamp: Optional[int] = None):
        self.seed = seed
        self.random = random.Random(seed)
        # Default base timestamp: 2024-01-01 00:00:00 UTC
        self.base_timestamp = base_timestamp if base_timestamp is not None else 1704067200000
    ...
    def generate_ticks(self, instrument_symbol: str, count: int = 1000) -> List[Dict[str, Any]]:
        ...
        for i in range(count):
            ...
            tick = {
                ...
                "timestamp": self.base_timestamp + (i * 1000),
            }
            ticks.append(tick)
    ...
    def generate_candles(self, instrument_symbol: str, interval_minutes: int = 60,
        ...
        for i in range(count):
            ...
            candle = {
                "instrument": instrument_symbol,
                "time": self.base_timestamp + (i * interval_ms),
    ...
    def export_json(self, filepath: str, data: Any, include_metadata: bool = True):
        output = data
        if include_metadata and isinstance(data, (list, dict)):
            # Use deterministic timestamp based on base_timestamp for reproducibility
            generated_at = datetime.fromtimestamp(self.base_timestamp / 1000, tz=timezone.utc).isoformat()
            output = {
                "_metadata": {
                    "seed": self.seed,
    ...
    def export_csv(self, filepath: str, data: List[Dict], fieldnames: Optional[List[str]] = None):
        ...
        with open(filepath, "w", newline="") as f:
            # Write seed metadata as comment header
...
def parse_args():
    parser = argparse.ArgumentParser(description="Test data generator with deterministic seed support")
    ...
    parser.add_argument("--print-seed", action="store_true", help="Print the seed used (useful when seed is randomly generated)")
    parser.add_argument("--base-timestamp", type=int, default=None, help="Base timestamp in milliseconds (defaults to 2024-01-01 00:00:00 UTC)")
...
def main():
    ...
    # Print seed when requested or when randomly generated
    if args.print_seed or args.seed is None:
        print(f"Seed: {seed}")

    gen = DataGenerator(seed, args.base_timestamp)
